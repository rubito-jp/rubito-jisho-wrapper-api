...
function getKanjiAndKana(div) {
  // div is a DOM Element (a .sentence_content)
  const ul = div.querySelector("ul");
  if (!ul) return { kanji: "", kana: "" };
  ...
  for (const node of ul.childNodes) {
    if (node.nodeType === 1 && node.tagName === "LI") {
      const furigana = node.querySelector(".furigana")?.textContent || "";
      const unlifted = node.querySelector(".unlinked")?.textContent || "";

      if (furigana) {
        kanji += unlifted;
        kana += furigana;

        // handle kana ending (same logic as original)
        const kanaEnding = [];
        for (let j = unlifted.length - 1; j > 0; j--) {
          if (!unlifted[j].match(/[\u4e00-\u9faf\u3400-\u4dbf々]/)) {
...
function normalizeSentenceElement(sentenceHtml) {
  ...
  const { document } = parseHTML(normalizedSentenceHtml);
  return document;
}

function getPieces(sentenceElement) {
  // sentenceElement is a DOM Element
  const doc = normalizeSentenceElement(sentenceElement.innerHTML); 
  const pieceElements = [...doc.querySelectorAll("li.clearfix")];

  return pieceElements.map(pieceElement => ({
    lifted: pieceElement.querySelector(".furigana")?.textContent.trim() || "",
    unlifted: pieceElement.querySelector(".unlinked")?.textContent.trim() || "",
  }));
}


function parseExampleDiv(div) {
  // `div` is a DOM Element from linkedom
  const english = div.querySelector(".english")?.textContent.trim() || "";
...
function parseExamplePageData(pageHtml, phrase) {
  ...
  const divs = [...document.querySelectorAll(".sentence_content")];

  const results = divs.map(div => parseExampleDiv(div));
...
function getTags(document) {
  return [...document.querySelectorAll(".concept_light-tag")]
    .map(el => el.textContent.trim());
}

 function getMeaningsOtherFormsAndNotes(document) {
  const returnValues = { otherForms: [], notes: [] };

  const meaningsWrapper = document.querySelector(
    "#page_container > div > div > article > div > div.concept_light-meanings.medium-9.columns > div"
  );
  if (!meaningsWrapper) return returnValues;
  ...
  for (const child of meaningsChildren) {
    if (child.classList.contains("meaning-tags")) {
      // Track current meaning section
      mostRecentWordTypes = child.textContent
        .split(",")
        .map(s => s.trim().toLowerCase());
    } 
    else if (mostRecentWordTypes[0] === "other forms") {
      // Other forms like: 漢字【かんじ】
      returnValues.otherForms = child.textContent.split("、")
        .map(s => s.replace("【", "").replace("】", "").split(" "))
        .map(a => ({ kanji: a[0], kana: a[1] }));
    } 
    else if (mostRecentWordTypes[0] === "notes") {
      // Notes (list of strings)
      returnValues.notes = child.textContent.split("\n").map(s => s.trim()).filter(Boolean);
    } 
    else {
      // Meaning section
      const meaning = child.querySelector(".meaning-meaning")?.textContent.trim() || "";
      const meaningAbstract = child.querySelector(".meaning-abstract")?.textContent.trim() || "";

      const rawSupplemental = [...child.querySelectorAll(".supplemental_info")]
        .map(el => el.textContent.trim())
        .filter(Boolean);
      ...
      for (const entry of rawSupplemental) {
        if (entry.startsWith("See also")) {
          seeAlsoTerms.push(entry.replace("See also ", ""));
        } else {
      ...
      const sentences = [...child.querySelectorAll(".sentences .sentence")].map(sentenceElement => {
        const english = sentenceElement.querySelector(".english")?.textContent.trim() || "";
        const japanese = sentenceElement.textContent.trim();

        const pieces = getPieces(sentenceElement); // already ported for Linkedom

        return { english, japanese, pieces };
      });
...
function getAudio(document) {
  return [...document.querySelectorAll(".concept_light-status audio > source")]
    .map(source => ({
      uri: `https:${source.getAttribute("src")}`,
...
function parsePhrasePageData(pageHtml, query) {
  ...
  const { meanings, otherForms, notes } = getMeaningsOtherFormsAndNotes(document);
  const audio = getAudio(document);
